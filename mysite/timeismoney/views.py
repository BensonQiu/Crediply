...
def notInTime(startTime):
	startHour = int(startTime.split(":")[0])
	startMinute = int(startTime.split(":")[1])
	currentHour = datetime.datetime.now().hour
	currentMinute = datetime.datetime.now().minute

	if currentHour > startHour or (currentHour == startHour and currentMinute > startMinute):
		return "true"
	return "false"

@login_required
def summary(request, meetingName):
	meetingTime = Meeting.objects.filter(meetingName=meetingName)[0].startDT
	formatTime = meetingTime[10:12] + ':' + meetingTime[13:15]
	isLate = notInTime(formatTime)
	context = {
		'isLate': isLate,
	}
